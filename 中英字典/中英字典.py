...
barber n.理发师
bare a.赤裸的；仅仅的
bargain n.交易 vi.议价； bark n.吠叫声 vi.吠，叫
barn n.谷仓；牲口棚
barrel n.桶；圆筒；枪管
barrier n.栅栏，屏障；障碍
base n.基础，底层；基地
...
dd = {}
for i in data.split('\n'):
    e,c = i.split(' ')[0],' '.join(i.split(' ')[1:])
    d[e] = c
    dd[c] = e
def f1(c,e):
    d[c] = e
    tk.messagebox.showinfo(title='结果', message='输入成功')
def f2(c):
    tk.messagebox.showinfo(title='结果', message=d[c])
def f3(c):
    tk.messagebox.showinfo(title='结果', message=dd[c])
def f4(c,e):
    d[c] = e
    tk.messagebox.showinfo(title='结果', message='修改成功')
e11 = Entry(root)
# e11.place(x=150,y=70)
e12 = Entry(root)
# e12.place(x=350,y=70)
e21 = Entry(root)
e21.place(x=150,y=170)
e31 = Entry(root)
e31.place(x=150,y=270)
e41 = Entry(root)
# e41.place(x=150,y=370)
e42 = Entry(root)
# e42.place(x=350,y=370)
# Button(root, width=13, height=2, text='输入', command=lambda:f1(e11.get(),e12.get())).place(x=20,y=50)
Button(root, width=13, height=2, text='查询', command=lambda:f2(e21.get())).place(x=20,y=150)
Button(root, width=13, height=2, text='查询', command=lambda:f3(e31.get())).place(x=20,y=250)
# Button(root, width=13, height=2, text='修改', command=lambda:f4(e41.get(),e42.get())).place(x=20,y=350)

# Label(root,text='英文意思:').place(x=150,y=45)
# Label(root,text='中文意思:').place(x=350,y=45)
Label(root,text='英文意思:').place(x=150,y=145)
Label(root,text='中文意思:').place(x=150,y=245)
# Label(root,text='英文意思:').place(x=150,y=345)
# Label(root,text='中文意思:').place(x=350,y=345)
root.mainloop()