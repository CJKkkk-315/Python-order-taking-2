...
data = sio.loadmat(dir_path+ "matlab")
dis_diff_start = data['dis_diff_start']
end = data['end_port']
label = data['label']
k = 0
f = []
for i in range(100):
    ff = []
    for j in range(3):
        temp = (dis_diff_start[k:k + 4].tolist(), end[k:k + 4].tolist(), label[i + j])
        ff.append(temp)
        k += 4
    f.append(ff)
...
np.savez(dir_path+"label.npz",
         train=f, train_periods=range(0, 100),
         valid=fv, valid_periods=range(100, 200),
         test=ft, test_periods=range(200, 300))
...
def make_links_info(nodes_num):
    ...
    features_type = {"fes_1": {"float": 1}, "fes_2": {"float": 1}, "fes_3": {"one_hot": 11459, "start": 0},
                     "fes_4": {"one_hot": 11459, "start": 1}, "fes_5": {"one_hot": 11459, "start": 0},
                     ...
                     "fes_15": {"one_hot": 11459, "start": 0}, "fes_16": {"one_hot": 11459, "start": 0}}
    link_info_list = []
