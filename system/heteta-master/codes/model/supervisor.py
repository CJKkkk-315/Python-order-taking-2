...
from codes.util.AMSGrad import AMSGrad
from codes.util.utils import tprint, clear_dir
from codes.util.support import *
from codes.model.HetETA_model import HetETAModel
...
class Supervisor(object):
    ...
    def __init__(self, data, adj_mx, config):
        ...
        self._length_supports = len(list(self.load_adj(adj_mx, self._method)))
        ...
        self._lr = tf.compat.v1.train.exponential_decay(self._base_lr, global_step,
        ...
        # a = len(list(self._supports))
        self.models = []
        for gpu_id in range(self._num_gpu):
            with tf.device('/gpu:%d' % gpu_id):
                print('\t Initing tower: %d...'% gpu_id)
                with tf.name_scope('tower_%d' % gpu_id):
                    a_model = self.get_model(config, self._length_supports, reuse=tf.compat.v1.AUTO_REUSE)
        ...
        self._saver = tf.compat.v1.train.Saver(tf.compat.v1.global_variables(), max_to_keep=self.max_to_keep)
    ...
    def load_adj(self, adj, method="HetETA"):
        ...
        if method in ["HetETA"]:
            ...
            # print("the num of supports", len(list(supports)[0]))
            # print(len(list(supports)))
            # print("the num of atten_supports", len(list(atten_supports)[0]))
        else:
            supports = []
    ...
    def get_optimizer(self, optimizer_name, epsilon):
        if optimizer_name == 'adam':
            ...
            optimizer = tf.compat.v1.train.RMSPropOptimizer(self._lr, epsilon=epsilon)
        else:
        ...
        with tf.compat.v1.variable_scope('MODEL', reuse=reuse):
            if self._method == 'HetETA':
                if self._atten_supports:
                    model = HetETAModel(config, supports_num, len(list(self._atten_supports)), cpu_supports, self.optimizer)
                else:
    ...
    def train(self, sess):
        ...
        best_model = ""
        wait = 0
        saver = tf.compat.v1.train.Saver(tf.compat.v1.global_variables(), max_to_keep=self.max_to_keep)
