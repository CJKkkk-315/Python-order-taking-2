...
from codes.model.cell.cheb_layer import STConvBlock, STLastLayer, STPredictLayer
...
class HetETAModel(object):
    def __init__(self, config, supports_num, atten_supports_num, cpu_supports, optimizer):
        ...
        self._ph = {
            'features': tf.compat.v1.placeholder(tf.float32, shape=(input_len, num_nodes, fe_dim), name='features'),
            'supports': [tf.compat.v1.sparse_placeholder(tf.float32, name='supports_%d' % i) for i in range(supports_num)],
            'atten_supports': [tf.compat.v1.sparse_placeholder(tf.float32, name='atten_supports_%d' % i) for i in range(atten_supports_num)],
            'batch_link_move_sp': tf.compat.v1.sparse_placeholder(tf.float32, shape=(None, num_nodes), name='batch_link_move_sp'),
            'batch_sum_time': tf.compat.v1.placeholder(tf.float32, shape=None, name='batch_sum_time'),
            'dropout': tf.compat.v1.placeholder_with_default(0., shape=None, name='dropout'),
        }
        ...
        reg_ws1 = 0.
        var_lists1 = tf.compat.v1.get_collection(tf.GraphKeys.WEIGHTS)
        for var in var_lists1:
            reg_ws1 += tf.nn.l2_loss(var)
        var_lists2 = tf.compat.v1.get_collection("weight_decay")
