import math
import numpy as np
def BSD(H,a,p,c,q,Kh,Kv,FS):
    a=math.radians(a)
    q=math.radians(q)
    Xl = 0
    Xr = np.arange(H / (math.tan(a)), H / (math.tan(a)) + H, 1.0)
    Xo = np.arange(-H, H, 1.0)
    fs = np.zeros((len(Xr) ,len(Xo)))
    for i in range(len(Xr)):
        for j in range(len(Xo)):
            fs[i][j]=BISHOPD(H,a,p,c,q,Kh,Kv,FS,Xl,Xr[i],Xo[j])
...
def BISHOPD(H, a, p, c, q, Kh, Kv, FS, Xl, Xr, Xo):
